...
Make sure that if you are using Linux you have ``xclip`` or ``xsel`` commands. Otherwise run ``sudo apt install xclip`` or ``sudo apt install xsel``. Note: ``xsel`` does not always seem to work. Otherwise on Linux, you need the gtk or PyQt4 modules installed.
